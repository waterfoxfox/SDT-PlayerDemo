...
BOOL CSDClient::Start()
{
	...
	bRet = SDVideoRender_AddStream(m_pVideoRender, g_byRecvPosition, 0, 0.0, 0.0, 1.0, 1.0);
	if (bRet == FALSE)
	{
	...
	if (g_bSaveDownTs == TRUE)
	{
		...
		SDTerminal_StartCapDownStream(m_pTerminal, g_byRecvPosition, acSaveFileDir);
	}
	...
	if (g_bSaveDownRaw == TRUE)
	{
		...
		SDTerminal_StartCapDownRawStream(m_pTerminal, g_byRecvPosition, acSaveFileDir);
	}
...
void CSDClient::Stop()
{
	...
	if (g_bSaveDownTs == TRUE)
	{
		SDTerminal_StopCapDownStream(m_pTerminal, g_byRecvPosition);
	}

	if (g_bSaveDownRaw == TRUE)
	{
		SDTerminal_StopCapDownRawStream(m_pTerminal, g_byRecvPosition);
	}
